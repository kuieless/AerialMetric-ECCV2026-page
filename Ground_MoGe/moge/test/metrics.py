...
def delta_depth_threshold(
    ...
    gt = gt.clamp_min(eps)
    ratio = torch.maximum(gt / pred, pred / gt)
    if inclusive:
...
def _sliding_window_2d_compat(x: torch.Tensor, window_size: int) -> torch.Tensor:
    """
    Compatibility wrapper for utils3d versions that do not expose `sliding_window_2d`.
    """
    sliding_window = getattr(utils3d.pt, "sliding_window_2d", None)
    if callable(sliding_window):
        return sliding_window(x, window_size=window_size, stride=1, dim=(-2, -1))
    ...
    if h < window_size or w < window_size:
        raise ValueError(
            f"window_size={window_size} is larger than input shape ({h}, {w})"
        )

    # torch.nn.functional.unfold requires floating-point inputs.
    input_is_bool = x.dtype == torch.bool
    x_f = x.float() if input_is_bool else x
    patches = F.unfold(x_f[None, None], kernel_size=(window_size, window_size), stride=1)
    out_h, out_w = h - window_size + 1, w - window_size + 1
    patches = (
        patches.view(1, window_size * window_size, out_h, out_w)
        .permute(0, 2, 3, 1)
        .reshape(out_h, out_w, window_size, window_size)
    )
    if input_is_bool:
        patches = patches > 0.5
    return patches
...
def boundary_f1(pred: torch.Tensor, gt: torch.Tensor, mask: torch.Tensor, radius: int = 1):
    ...
    pred_window = _sliding_window_2d_compat(pred, window_size=window_size)       # [H, W, 2*R+1, 2*R+1]
    gt_window = _sliding_window_2d_compat(gt, window_size=window_size)           # [H, W, 2*R+1, 2*R+1]
    mask_window = neighbor_mask & _sliding_window_2d_compat(mask, window_size=window_size)  # [H, W, 2*R+1, 2*R+1]
...
def compute_metrics(
    ...
    #### Required keys in gt:
        ...
        - `depth_mask`: mask indicating valid pixels in the ground truth.
        - `is_metric`: whether the depth is in metric units.

    #### Optional keys in gt:
        - `points`: point map ground truth in camera coordinates. Required only when computing point metrics.
        - `intrinsics`: normalized ground-truth intrinsics. Required for FOV metric and depth-to-point visualization.

    #### Args:
        - `metric_depth_only`: if True, compute only `depth_metric` metrics and skip all other metrics.
    """
    metrics = {}
    misc = {}
    ...
    def _build_depth_metrics(pred_depth_masked: torch.Tensor, gt_depth_masked: torch.Tensor) -> Dict[str, float]:
        return {
            'absrel': rel_depth(pred_depth_masked, gt_depth_masked),
            'rmse': rmse_depth(pred_depth_masked, gt_depth_masked),
            'delta1': delta_depth_threshold(
                pred_depth_masked,
                gt_depth_masked,
                threshold=1.25,
                inclusive=False,
            ),
            'delta2': delta_depth_threshold(
                pred_depth_masked,
                gt_depth_masked,
                threshold=1.25 ** 2,
                inclusive=False,
    ...
    if 'depth_metric' in pred and gt['is_metric']:
        ...
        metrics['depth_metric'] = _build_depth_metrics(pred_depth[mask], gt_depth[mask])

        if pred_depth_aligned is None:
            pred_depth_aligned = pred_depth

    if metric_depth_only:
        if vis and pred_depth_aligned is not None:
            if only_depth and 'intrinsics' in gt:
    ...
    if pred_depth_scale_invariant is not None:
        ...
        metrics['depth_scale_invariant'] = _build_depth_metrics(pred_depth[mask], gt_depth[mask])
    ...
    if pred_depth_affine_invariant is not None:
        ...
        metrics['depth_affine_invariant'] = _build_depth_metrics(pred_depth[mask], gt_depth[mask])
    ...
    if pred_disparity_affine_invariant is not None:
        ...
        metrics['disparity_affine_invariant'] = _build_depth_metrics(pred_depth[mask], gt_depth[mask])

        if pred_depth_aligned is None:
            pred_depth_aligned = 1 / pred_disp.clamp_min(1e-6)

    # Metric points
    if 'points_metric' in pred and gt['is_metric'] and gt_points is not None:
    ...
    if pred_points_scale_invariant is not None and gt_points is not None:
    ...
    if pred_points_affine_invariant is not None and gt_points is not None:
