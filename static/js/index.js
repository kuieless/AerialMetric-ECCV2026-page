...
const DEPTH_EXPLORER_CAMERA = {
  position: { x: 0, y: 0.03, z: -2.35 },
  target: { x: 0, y: 0, z: 0 },
  minDistance: 1.25,
  maxDistance: 6.0,
};
...
function initDepthExplorer() {
  ...
  function updateReadout() {
    ...
    if (activeLabel) activeLabel.textContent = `${point.id} · ${point.label}`;
    if (valueGt) valueGt.textContent = `${point.depths.gt.toFixed(1)} m`;
    if (valueBaseline) valueBaseline.textContent = `${point.depths.baseline.toFixed(1)} m`;
    if (valueLora) valueLora.textContent = `${point.depths.lora96.toFixed(1)} m`;
    if (cardGt) cardGt.classList.add('is-reference');
    if (cardBaseline) {
  ...
  function buildViewScaffold(modelKey) {
    ...
    view.camera.position.set(
      DEPTH_EXPLORER_CAMERA.position.x,
      DEPTH_EXPLORER_CAMERA.position.y,
      DEPTH_EXPLORER_CAMERA.position.z,
    );
    ...
    view.controls.minDistance = DEPTH_EXPLORER_CAMERA.minDistance;
    view.controls.maxDistance = DEPTH_EXPLORER_CAMERA.maxDistance;
    view.controls.target.set(
      DEPTH_EXPLORER_CAMERA.target.x,
      DEPTH_EXPLORER_CAMERA.target.y,
      DEPTH_EXPLORER_CAMERA.target.z,
    );
    view.controls.update();
    view.controls.addEventListener('change', () => syncCamera(modelKey));
  }

  function resetViewCamera(view) {
    view.camera.position.set(
      DEPTH_EXPLORER_CAMERA.position.x,
      DEPTH_EXPLORER_CAMERA.position.y,
      DEPTH_EXPLORER_CAMERA.position.z,
