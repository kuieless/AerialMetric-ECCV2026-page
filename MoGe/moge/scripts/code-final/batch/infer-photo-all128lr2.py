...
CHECKPOINT_ROOT = "/data1/szq/workspace/lora-batch128-256-lessground4-with-UElr2-2/checkpoint"
EVAL_STEP_INTERVAL = 3500      # 【核心】每隔多少步评估一次？设为 100 表示只跑 0, 100, 200... (设为 None 或 0 则关闭间隔筛选)
EVAL_EXACT_STEPS = []       # 【白名单】额外强制评估的特定步数，无视间隔规则。例如想看早期效果可以设为 [50] (不需要则设为 [])
...
TASK_A_OUTPUT_DIR = "/data1/szq/Infer-Final/Table2-1/Table2-head-LoRA-128-groundless4-UE3"
TASK_A_DATASETS = [
    "/data1/szq/Val/Bench",
...
TASK_B_OUTPUT_DIR = "/data1/szq/Table2-head-LoRA-128-Ground-groundless4-UE3"
TASK_B_BASELINE_SCRIPT = "/home/szq/moge2/MoGe/moge/scripts/eval_baseline_lora128.py"
TASK_B_EVAL_SCRIPT_REL = "moge/scripts/eval_baseline.py"
TASK_B_DATASET_CONFIG = "configs/eval/all_benchmarks.json"

# --- 任务 C: 绘图配置 ---
PLOT_ENABLE = True    
PLOT_SAVE_PREFIX = "final_lora128-groundless4-UE3" 
...
def process_single_ckpt_task_a(ckpt_path):
    ...
    for dataset_path in TASK_A_DATASETS:
        ...
        if has_files:
            ...
            try:
                run_cmd([
                    "python", "a-infer_lora128.py", # ⚠️ 依循原代码使用 a-infer_lora.py，如果有误可改为 a-infer_lora32.py
                    "--input", dataset_path,
                    "--output", infer_out,
                    ...
                    "--resize", "0",
                    "--batch_size", "16"  # <--- 🚀 加上这个参数，8或者16，看你的显存能不能撑住
