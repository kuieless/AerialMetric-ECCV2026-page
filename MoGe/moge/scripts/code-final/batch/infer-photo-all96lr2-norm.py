'''
python /home/szq/moge2/MoGe/moge/scripts/code-final/c-eval-oblique.py \
  --pred /data1/szq/Infer-Final/Table2-1/Table2-head-LoRA-96-groundless4-UElr2-norm/00001000/Extracted \
  --gt /data1/szq/Val/Oblique

python /home/szq/moge2/MoGe/moge/scripts/code-final/c-eval-bench.py \
  --pred /data1/szq/Table2-final-baseline-with-intrin2/vitl-normal/Extracted/Bench-ori \
  --gt /data1/szq/Val/Bench-ori
'''
...
EVAL_STEP_INTERVAL = 1000      # 【核心】每隔多少步评估一次？设为 100 表示只跑 0, 100, 200... (设为 None 或 0 则关闭间隔筛选)
EVAL_EXACT_STEPS = [0]       # 【白名单】额外强制评估的特定步数，无视间隔规则。例如想看早期效果可以设为 [50] (不需要则设为 [])
...
TASK_A_OUTPUT_DIR = "/data1/szq/Infer-Final/Table2-1/Table2-head-LoRA-96-groundless4-UElr2-norm2"
TASK_A_DATASETS = [
    "/data1/szq/Val/Bench-norm",
    "/data1/szq/Val/Oblique-norm"
]
...
def process_single_ckpt_task_a(ckpt_path):
    ...
    report_bench = os.path.join(extract_out, "Bench-norm", "Eval_Report_Bench.txt")
    report_oblique = os.path.join(extract_out, "Oblique-norm", "Eval_Report_Oblique_Pixel.txt")
    ...
    for dataset_path in TASK_A_DATASETS:
        ...
        if has_files:
            ...
            try:
                run_cmd([
                    "python", "a-infer_lora96-norm.py", # ⚠️ 依循原代码使用 a-infer_lora.py，如果有误可改为 a-infer_lora32.py
                    "--input", dataset_path,
                    "--output", infer_out,
    ...
    # 2. 抽取 (Extraction)
    need_extract = True
    if os.path.exists(os.path.join(extract_out, "Bench-norm")) and os.path.exists(os.path.join(extract_out, "Oblique")):
         if len(os.listdir(os.path.join(extract_out, "Bench-norm"))) > 0:
             need_extract = False
    ...
    if not os.path.exists(report_bench):
        bench_pred = os.path.join(extract_out, "Bench-norm")
