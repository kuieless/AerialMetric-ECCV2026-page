...
# 1. 权重根目录
CHECKPOINT_ROOT = "/home/szq/moge2/MoGe/workspace/lora-batch8-16-lessground4-onlyaerial/checkpoint"
...
TASK_A_OUTPUT_DIR = "/data1/szq/Infer-Final/Table2-1/Table2-head-LoRA-8-groundless4-no-ground-all"
TASK_A_DATASETS = [
    "/data1/szq/Val/Bench",
...
TASK_B_OUTPUT_DIR = "/data1/szq/Table2-head-LoRA-8-Ground-groundless4-no-ground-all"
...
# --- 任务 C: 绘图配置 ---
PLOT_ENABLE = True    
PLOT_SAVE_PREFIX = "final_lora8-groundless4-no-ground-all" 
...
def process_single_ckpt_task_a(ckpt_path):
    ...
    os.makedirs(base_dir, exist_ok=True) # 🔥 新增：确保权重子文件夹存在
    ...
    for dataset_path in TASK_A_DATASETS:
        ...
        if os.path.exists(expected_out) and len(glob.glob(os.path.join(expected_out, "*.npy"))) > 0:
            ...
            try:
                run_cmd(["python", "a-infer_lora8.py", "--input", dataset_path, "--output", infer_out, "--config", LORA_CONFIG_PATH, "--weight", ckpt_path, "--ratio", str(SAMPLING_RATIO), "--resize", "0",
                    "--batch_size", "8"], log_file=run_log, cwd=TASK_A_SCRIPT_DIR, env=env)
            except Exception as e:
                print(f"      ❌ Task A 推理失败: {e}")

    # 2. 抽取阶段
    need_extract = not (os.path.exists(os.path.join(extract_out, "Bench")) and os.path.exists(os.path.join(extract_out, "Oblique")) and len(os.listdir(os.path.join(extract_out, "Bench"))) > 0)
    if not need_extract:
        print(f"      ⏩ 抽取文件存在，跳过。")
    else:
        try:
            run_cmd(["python", "b-extra.py", "--input", infer_out, "--output", extract_out, "--target", "depth.npy", "--ext", ".npy"], log_file=run_log, cwd=TASK_A_SCRIPT_DIR, env=env)
        except Exception as e:
            print(f"      ❌ Task A 抽取失败: {e}")

    # 3. 评估阶段
    if not os.path.exists(report_bench) and os.path.exists(os.path.join(extract_out, "Bench")):
        try: run_cmd(["python", "c-eval-bench.py", "--pred", os.path.join(extract_out, "Bench"), "--gt", "/data1/szq/Val/Bench"], log_file=run_log, cwd=TASK_A_SCRIPT_DIR, env=env)
        except: pass
    if not os.path.exists(report_oblique) and os.path.exists(os.path.join(extract_out, "Oblique")):
        try: run_cmd(["python", "c-eval-oblique.py", "--pred", os.path.join(extract_out, "Oblique"), "--gt", "/data1/szq/Val/Oblique"], log_file=run_log, cwd=TASK_A_SCRIPT_DIR, env=env)
        except: pass
    ...
    if os.path.exists(report_bench) and os.path.exists(report_oblique):
        ...
        # 4.2 遍历 Extracted 文件夹，删除所有非 .txt 的文件 (如 depth.npy)
...
def process_single_ckpt_task_b(ckpt_path):
    ...
    try:
        run_cmd(["python", TASK_B_EVAL_SCRIPT_REL, "--baseline", TASK_B_BASELINE_SCRIPT, "--config", TASK_B_DATASET_CONFIG, "--output", output_json, "--lora_config", LORA_CONFIG_PATH, "--lora_weight", ckpt_path, "--resolution_level", "9", "--ratio", str(SAMPLING_RATIO)], log_file=run_log, cwd=TASK_B_PROJECT_ROOT, env=env)
    except Exception as e:
...
def parse_txt_file(file_path, source_type, step):
    ...
    targets = ["OVERALL", "Cleaned_Dataset_Campus", "Cleaned_Dataset_Factory", "Cleaned_Dataset_Farm", "Cleaned_Dataset_Gress"] if source_type == "Bench" else ["OVERALL", "[CAT] City", "[CAT] Natural", "[CAT] Rural"]
    for line in lines:
        ...
        try:
            rows.append({"Step": step, "Source": source_type, "Scene": clean_txt_scene_name(raw_name), "AbsRel": float(parts[2]), "RMSE": float(parts[3]), "a1.25": float(parts[5]), "Type": "Summary" if clean_txt_scene_name(raw_name).upper() == "OVERALL" else "Detail"})
        except: continue
    return rows
...
def parse_json_file(file_path, step):
    ...
    try:
        ...
        for ds_name, metrics_group in content.items():
            ...
            rows.append({"Step": step, "Source": "Depth_JSON", "Scene": "Mean" if is_summary else ds_name, "AbsRel": float(metrics.get("rel", 0)), "RMSE": float(metrics.get("rmse", 0)), "a1.25": float(metrics.get("delta1", 0)), "Type": "Summary" if is_summary else "Detail"})
    except: pass
    return rows
...
def run_plotting():
    ...
    def plot_metric(metric, filename, lower_better=True):
        ...
        if not df_sum.empty:
            ...
            sns.lineplot(data=df_sum, x="Step", y=metric, hue="Legend", style="Legend", markers=True, linewidth=3.5, markersize=10, ax=axes[0])
            suffix = "↓" if lower_better else "↑"
            axes[0].set_title(f"Global Summary ({metric} {suffix})", fontsize=16, fontweight='bold', color='darkred') # 🔥 移除了表情符号避免字体警告
...
if __name__ == "__main__":
    ...
    for i, ckpt in enumerate(checkpoints):
        ...
        if step == 0:
            ...
            print(f"{'='*80}")
            # 遇到 0 直接调用画图，这样你第一时间就能看到基准点，不需要进行 Task A 和 B
            if PLOT_ENABLE:
...
# # 1. 权重根目录
# CHECKPOINT_ROOT = "/home/szq/moge2/MoGe/workspace/lora-batch8-16-lessground4/checkpoint"
...
# TASK_A_ENABLE = True 
# TASK_A_SCRIPT_DIR = "/home/szq/moge2/MoGe/moge/scripts/code-final"
# TASK_A_OUTPUT_DIR = "/data1/szq/Infer-Final/Table2-1/Table2-head-LoRA-8-groundless4-no-UE-all"
# TASK_A_DATASETS = [
#     "/data1/szq/Val/Bench",
#     "/data1/szq/Val/Oblique"
# ]
...
# TASK_B_ENABLE = True 
# TASK_B_PROJECT_ROOT = "/home/szq/moge2/MoGe"
# TASK_B_OUTPUT_DIR = "/data1/szq/Table2-head-LoRA-8-Ground-groundless4-no-UE-all"
# TASK_B_BASELINE_SCRIPT = "/home/szq/moge2/MoGe/moge/scripts/eval_baseline_lora8.py"
# TASK_B_EVAL_SCRIPT_REL = "moge/scripts/eval_baseline.py"
# TASK_B_DATASET_CONFIG = "configs/eval/all_benchmarks.json"
...
# # --- 任务 C: 绘图配置 ---
# PLOT_ENABLE = True    
# PLOT_SAVE_PREFIX = "final_lora8-groundless4-with-UE-all" 
...
#         # ================= Ground JSON (0 Step) =================
#         # 注意: 截图中未提供 RMSE，用 None 替代，Seaborn 会自动忽略不画点
#         {"Step": 0, "Source": "Depth_JSON", "Scene": "NYUv2", "AbsRel": 0.0689, "RMSE": 0.23, "a1.25": 0.967, "Type": "Detail"},
#         {"Step": 0, "Source": "Depth_JSON", "Scene": "KITTI", "AbsRel": 0.1750, "RMSE": 4.26, "a1.25": 0.647, "Type": "Detail"},
#         {"Step": 0, "Source": "Depth_JSON", "Scene": "ETH3D", "AbsRel": 0.0998, "RMSE": 0.74, "a1.25": 0.888, "Type": "Detail"},
#         {"Step": 0, "Source": "Depth_JSON", "Scene": "iBims", "AbsRel": 0.1450, "RMSE": 0.54, "a1.25": 0.804, "Type": "Detail"},
...
# #         else:
# #             try:
# #                 run_cmd(["python", "a-infer_lora8.py", "--input", dataset_path, "--output", infer_out, "--config", LORA_CONFIG_PATH, "--weight", ckpt_path, "--ratio", str(SAMPLING_RATIO), "--resize", "0"], cwd=TASK_A_SCRIPT_DIR, env=env)
# #             except Exception as e:
# #                 print(f"      ❌ Task A 推理失败: {e}")
...
# #     else:
# #         try:
# #             run_cmd(["python", "b-extra.py", "--input", infer_out, "--output", extract_out, "--target", "depth.npy", "--ext", ".npy"], cwd=TASK_A_SCRIPT_DIR, env=env)
# #         except Exception as e:
# #             print(f"      ❌ Task A 抽取失败: {e}")

# #     if not os.path.exists(report_bench) and os.path.exists(os.path.join(extract_out, "Bench")):
# #         try: run_cmd(["python", "c-eval-bench.py", "--pred", os.path.join(extract_out, "Bench"), "--gt", "/data1/szq/Val/Bench"], cwd=TASK_A_SCRIPT_DIR, env=env)
# #         except: pass
# #     if not os.path.exists(report_oblique) and os.path.exists(os.path.join(extract_out, "Oblique")):
# #         try: run_cmd(["python", "c-eval-oblique.py", "--pred", os.path.join(extract_out, "Oblique"), "--gt", "/data1/szq/Val/Oblique"], cwd=TASK_A_SCRIPT_DIR, env=env)
# #         except: pass
# def process_single_ckpt_task_a(ckpt_path):
#     ckpt_name = Path(ckpt_path).stem
#     print(f"\n   📍 [Task A] 处理权重: {ckpt_name}")
...
#         else:
#             try:
#                 run_cmd(["python", "a-infer_lora8.py", "--input", dataset_path, "--output", infer_out, "--config", LORA_CONFIG_PATH, "--weight", ckpt_path, "--ratio", str(SAMPLING_RATIO), "--resize", "0"], log_file=run_log, cwd=TASK_A_SCRIPT_DIR, env=env)
#             except Exception as e:
#                 print(f"      ❌ Task A 推理失败: {e}")
