...
CHECKPOINT_ROOT = "/data1/szq/workspace/lora-batch64-128-with-UElr2/checkpoint"
EVAL_STEP_INTERVAL = 100      # 【核心】每隔多少步评估一次？设为 100 表示只跑 0, 100, 200... (设为 None 或 0 则关闭间隔筛选)
EVAL_EXACT_STEPS = [50]       # 【白名单】额外强制评估的特定步数，无视间隔规则。例如想看早期效果可以设为 [50] (不需要则设为 [])
...
TASK_A_OUTPUT_DIR = "/data1/szq/Infer-Final/Table2-1/Table2-head-LoRA-64-groundless4-UElr2"
TASK_A_DATASETS = [
    "/data1/szq/Val/Bench",
...
TASK_B_OUTPUT_DIR = "/data1/szq/Table2-head-LoRA-64-Ground-groundless4-UElr2"
TASK_B_BASELINE_SCRIPT = "/home/szq/moge2/MoGe/moge/scripts/eval_baseline_lora64.py"
TASK_B_EVAL_SCRIPT_REL = "moge/scripts/eval_baseline.py"
TASK_B_DATASET_CONFIG = "configs/eval/all_benchmarks.json"

# --- 任务 C: 绘图配置 ---
PLOT_ENABLE = True    
PLOT_SAVE_PREFIX = "final_lora64-groundless4-UElr2" 
...
def process_single_ckpt_task_a(ckpt_path):
    ...
    for dataset_path in TASK_A_DATASETS:
        ...
        if has_files:
            ...
            try:
                run_cmd([
                    "python", "a-infer_lora64.py", # ⚠️ 依循原代码使用 a-infer_lora.py，如果有误可改为 a-infer_lora32.py
                    "--input", dataset_path,
                    "--output", infer_out,
                    ...
                    "--resize", "0",
                    "--batch_size", "32"  # <--- 🚀 加上这个参数，8或者16，看你的显存能不能撑住
