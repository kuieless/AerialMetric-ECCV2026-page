...
CHECKPOINT_ROOT = "/data1/szq/workspace/final-head/checkpoint"

# --- 🎯 权重筛选配置 (人性化读取规则) ---
EVAL_STEP_INTERVAL =1200      # 【核心】每隔多少步评估一次？设为 100 表示只跑 0, 100, 200... (设为 None 或 0 则关闭)
EVAL_EXACT_STEPS = [50]       # 【白名单】额外强制评估的特定步数，无视间隔规则 (不需要则设为 [])
...
TASK_A_OUTPUT_DIR = "/data1/szq/Table2-final-head2-EMA" 
TASK_A_DATASETS = [
    "/data1/szq/Val/Bench",
...
TASK_B_OUTPUT_DIR = "/data1/szq/Table2-final-head2-EMA-Ground"
...
# --- 任务 C: 绘图配置 ---
PLOT_ENABLE = True    
PLOT_SAVE_PREFIX = "final_head2_ema" 
        ...
        if has_files:
            ...
            try:
                run_cmd([
                    ...
                    "--resize", "0",
                    "--batch_size", "32"   # 引入 LoRA 脚本中的批处理优化
