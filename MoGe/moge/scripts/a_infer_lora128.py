...
class MogeLoRAEngine:
    def __init__(self, config_path, lora_path, device="cuda", fp16=True):
        ...
        peft_config = LoraConfig(
            r=128, lora_alpha=256, bias="none",
...
# def setup_moge_path():
#     """自动寻找并添加 moge 包路径"""
#     # 假设当前脚本在 moge/scripts/code-final/ 目录下
#     # 我们向上寻找直到找到包含 'moge' 文件夹的目录
#     current_path = Path(__file__).resolve()
#     for parent in [current_path.parents[0], current_path.parents[1], current_path.parents[2]]:
#         if (parent / "moge").exists():
#             if str(parent) not in sys.path:
...
#     def run_scene(self, input_path, output_path, stride=1, resize=1024):
#         input_path = Path(input_path)
#         output_path = Path(output_path)
#         valid_exts = {'.jpg', '.png', '.jpeg', '.JPG', '.PNG'}
#         image_paths = sorted([p for p in input_path.iterdir() if p.suffix in valid_exts])
...
#         success_count = 0
#         with torch.inference_mode():
#             for img_path in tqdm(image_paths, desc=f"Scene: {input_path.name}", leave=False):
#                 try:
#                     self._process_one(img_path, input_path, output_path, resize)
#                     success_count += 1
#                 except Exception as e:
#                     print(f"❌ Error {img_path.name}: {e}")
#         return success_count

#     def _process_one(self, img_path, root_in, root_out, resize_to):
#         img_bgr = cv2.imread(str(img_path))
...
#                 process_bgr = cv2.resize(img_bgr, (new_w, new_h), interpolation=cv2.INTER_AREA)
#             else:
#                 # 如果原图比指定尺寸还小，也需要对齐 14
#                 new_w = (w_orig // 14) * 14
#                 new_h = (h_orig // 14) * 14
#                 if (new_w, new_h) != (w_orig, h_orig):
#                     process_bgr = cv2.resize(img_bgr, (new_w, new_h), interpolation=cv2.INTER_AREA)
#         else:
#             # 逻辑 B: 原图模式 (resize=None)
#             # ViT 模型要求输入必须能被 14 整除
#             new_w = (w_orig // 14) * 14
#             new_h = (h_orig // 14) * 14
            
#             # 只有当原图尺寸不能被 14 整除时，才进行微调
#             if (new_w, new_h) != (w_orig, h_orig):
#                 # print(f"⚠️ 微调尺寸以适配模型: {w_orig}x{h_orig} -> {new_w}x{new_h}")
#                 process_bgr = cv2.resize(img_bgr, (new_w, new_h), interpolation=cv2.INTER_AREA)
#         # ================= 🔥 核心修改结束 🔥 =================
        
#         img_rgb = cv2.cvtColor(process_bgr, cv2.COLOR_BGR2RGB)
#         tensor = torch.from_numpy(img_rgb).float().div(255.0).permute(2, 0, 1).unsqueeze(0).to(self.device)
#         if self.fp16: tensor = tensor.half()
        
#         # 推理
#         if hasattr(self.model, 'infer'): output = self.model.infer(tensor)
#         elif hasattr(self.model.base_model.model, 'infer'): output = self.model.base_model.model.infer(tensor)
#         else: raise AttributeError("无法找到 infer 方法")
            
#         depth = output['depth'].squeeze().cpu().numpy()
        
#         # 后处理：如果因为对齐 14 或者缩放导致尺寸变了，最后强制拉回原图尺寸
#         # 这样你的输出 depth.npy 永远和原始 jpg 一模一样大
#         if depth.shape != (h_orig, w_orig):
#             depth = cv2.resize(depth, (w_orig, h_orig), interpolation=cv2.INTER_LINEAR)
            
#         # 保存
#         save_dir = root_out / img_path.stem
#         save_dir.mkdir(parents=True, exist_ok=True)
#         np.save(str(save_dir / 'depth.npy'), depth)
...
#     对外暴露的主函数
#     Args:
#         input_roots (list): 输入数据集根目录列表
#         output_root (str): 输出根目录
#         lora_config (str): LoRA config.json 路径
#         lora_weight (str): LoRA .pt 权重路径
#         sampling_ratio (float): 采样率 (0.0~1.0)
#         resize (int): 缩放大小
#     """
#     # 1. 扫描
#     parser = DatasetAutoParser()
