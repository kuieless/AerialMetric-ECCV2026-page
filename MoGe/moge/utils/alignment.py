...
def split_batch_fwd(func, chunk_size, *args):
    """
    Split the batch into chunks and forward them one by one.
    Handles both Tensor arguments (sliced) and Scalar arguments (passed as-is).
    """
    # 假设第一个参数永远是 Tensor，用来确定 Batch Size
    if len(args) > 0 and isinstance(args[0], torch.Tensor):
        B = args[0].shape[0]
    else:
        # 如果第一个参数都不是 Tensor，这函数没法用了，直接硬跑
        return func(*args)
    
    # 🔥 [Fix 1] 处理空 Batch 的情况 (防止 IndexError)
    if B == 0:
        try:
            return func(*args)
        except:
            # 返回安全的默认值 (适配 MoGe 的 align 返回格式: scale, loss, index)
            dev = args[0].device
            return torch.zeros(0, device=dev), torch.zeros(0, device=dev), None

    # 如果 Batch 很小，直接运行，不用切分
    if B <= chunk_size:
        return func(*args)
    
    results = []
    for i in range(0, B, chunk_size):
        # 🔥 [Fix 2] 智能切分：只切 Tensor/List，保留 float/int
        chunk_args = []
        for arg in args:
            if isinstance(arg, (torch.Tensor, list, tuple)):
                # 如果是 tensor，切出一块
                chunk_args.append(arg[i:i + chunk_size])
            else:
                # 如果是 float/int (比如 trunc 参数)，直接原样传进去
                chunk_args.append(arg)
        
        # 执行函数
        results.append(func(*chunk_args))
    
    if len(results) == 0:
         dev = args[0].device
         return torch.zeros(B, device=dev), torch.zeros(B, device=dev), None
    ...
    if isinstance(results[0], tuple):
        ...
        for i in range(num_outputs):
            # 只有 Tensor 才能 cat，如果是 None (比如 index 有时是 None) 就跳过或保留 None
            if isinstance(results[0][i], torch.Tensor):
                outputs.append(torch.cat([res[i] for res in results], dim=0))
            else:
                outputs.append(results[0][i]) # 对于非 Tensor 返回值，通常不需要合并
