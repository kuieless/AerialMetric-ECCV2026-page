...
def solve_optimal_focal_shift(uv: np.ndarray, xyz: np.ndarray):
    "Solve `min |focal * xy / (z + shift) - uv|` with respect to shift and focal"
    from scipy.optimize import least_squares
    from functools import partial # 确保引入partial，虽然原代码可能在外部引入了
    
    uv, xy, z = uv.reshape(-1, 2), xyz[..., :2].reshape(-1, 2), xyz[..., 2].reshape(-1)

    def fn(uv: np.ndarray, xy: np.ndarray, z: np.ndarray, shift: np.ndarray):
        # 加上极小值防止除以0
        xy_proj = xy / (z + shift + 1e-8)[: , None]
        
        # 计算 focal
        num = (xy_proj * uv).sum()
        denom = np.square(xy_proj).sum() + 1e-8
        f = num / denom
    ...
    try:
        # --- [修改核心]：尝试求解，如果数据有毒(NaN/Inf)则捕获错误 ---
        solution = least_squares(partial(fn, uv, xy, z), x0=0, ftol=1e-3, method='lm')
        optim_shift = solution['x'].squeeze().astype(np.float32)

        # 重新计算最终的 focal
        xy_proj = xy / (z + optim_shift + 1e-8)[: , None]
        optim_focal = (xy_proj * uv).sum() / (np.square(xy_proj).sum() + 1e-8)

        return optim_shift, optim_focal

    except (ValueError, RuntimeError, Exception) as e:
        # --- [安全网]：如果报错，打印警告并返回默认值，防止训练崩溃 ---
        # 这种情况通常发生在训练初期，模型预测的 z 包含 NaN 或 Inf
        # print(f"[Warning] Visualization solver failed: {e}. Using default focal=1.0, shift=0.0")
