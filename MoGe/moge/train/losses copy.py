...
#     return weight
@torch.no_grad()  # <--- 🔥 关键修改：这部分不需要梯度，加这个能省巨量显存
def compute_anchor_sampling_weight(
    points: torch.Tensor, 
    mask: torch.Tensor, 
    radius_2d: torch.Tensor, 
    radius_3d: torch.Tensor, 
    num_test: int = 16  # <--- 🔥 关键修改：默认值从 64 改为 16
) -> torch.Tensor:
    # Importance sampling to balance the sampled probability of fine strutures.
...
def affine_invariant_local_loss(
    ...
    *batch_shape, height, width, _ = pred_points.shape
    ...
    # 🔥🔥🔥 关键修改：这里原来是 num_test=64，必须改成 16，否则前面改了没用 🔥🔥🔥
    anchor_sampling_weights = compute_anchor_sampling_weight(gt_points, gt_mask, radius_2d, radius_3d, num_test=16)
...
def multi_scale_gradient_loss(pred, gt, mask=None, params=None):
    ...
    # 🔥🔥🔥 自动调整维度：MoGe 的点云通常是 [B, H, W, C]，而梯度计算通常需要 [B, C, H, W]
    if pred.shape[-1] == 3 and pred.ndim == 4:
        # Permute to [B, C, H, W]
        pred = pred.permute(0, 3, 1, 2)
        gt = gt.permute(0, 3, 1, 2)
        if mask is not None:
             # Mask 通常是 [B, H, W]，扩充为 [B, 1, H, W]
            if mask.ndim == 3:
                mask = mask.unsqueeze(1)
            # 如果 Mask 本身没 Channel 维，不需要 permute，直接 unsqueeze 即可
    ...
    for scale in range(scales):
        ...
        # 检查 H 和 W 是否足够大切分
        # 现在 pred 是 [B, C, H, W]，所以 H是2，W是3
        if pred.shape[2] <= step or pred.shape[3] <= step:
            break

        # 计算 X 方向梯度 (Width方向，对应 dim=3)
        pred_grad_x = pred[..., :, step:] - pred[..., :, :-step]
        gt_grad_x = gt[..., :, step:] - gt[..., :, :-step]
        
        # 计算 Y 方向梯度 (Height方向，对应 dim=2)
        pred_grad_y = pred[..., step:, :] - pred[..., :-step, :]
        gt_grad_y = gt[..., step:, :] - gt[..., :-step, :]
        ...
        if mask is not None:
            ...
            # 广播 Mask 到 Channel 维度 (如果 mask 是单通道)
            valid_mask_x = mask_x.expand_as(gt_grad_x)
            valid_mask_y = mask_y.expand_as(gt_grad_y)
            
            if valid_mask_x.sum() > 0:
                loss_x = torch.abs(pred_grad_x[valid_mask_x] - gt_grad_x[valid_mask_x]).mean()
            else:
                loss_x = 0.0
                
            if valid_mask_y.sum() > 0:
                loss_y = torch.abs(pred_grad_y[valid_mask_y] - gt_grad_y[valid_mask_y]).mean()
            else:
                loss_y = 0.0
        else:
            loss_x = torch.abs(pred_grad_x - gt_grad_x).mean()
