...
class TrainDataLoaderPipeline:
    def __init__(self, config: dict, batch_size: int, num_load_workers: int = 4, num_process_workers: int = 4, buffer_size: int = 8):
        self.config = config
    ...
    #         # --- 修改开始：增加对 .npy 的支持 ---
    #         depth_filename = self.datasets[instance['dataset']].get('depth', 'depth.png')
    #         depth_path = Path(instance['path'], depth_filename)

    #         if str(depth_path).endswith('.npy'):
    #             # 如果是 npy，直接用 numpy 加载
    #             depth = np.load(depth_path)
    #             # 确保数据类型是 float32
    #             depth = depth.astype(np.float32)
    #             # 防止出现 (H, W, 1) 的维度，强制变为 (H, W)
    #             if depth.ndim == 3:
    #                 depth = depth.squeeze()
    #         else:
    #             # 如果是 png/exr，使用原来的读取函数
    ...
    #         # 打印更详细的错误堆栈，方便排查
    #         print(f"Failed to load instance {instance['dataset']}/{instance['filename']} because of exception:", e)
    #         traceback.print_exc() # 建议加上这一行，能看到具体哪一行报错
    #         instance.update(self.invalid_instance)
    #     return instance
    def _load_instance(self, instance: dict):
            try:
                # 1. 读取图片 (RGB)
                image_path = Path(instance['path'], 'image.jpg')
                image = read_image(image_path)
                
                # 2. 确定深度图路径 (支持配置指定文件名，默认为 depth.png)
                depth_filename = self.datasets[instance['dataset']].get('depth', 'depth.png')
                depth_path = Path(instance['path'], depth_filename)
                ...
                if str(depth_path).endswith('.npy'):
                    ...
                    # 🔥 [NPY 关键修复] 
                    # NPY 里通常用 0 表示无效，但在计算 Loss 时我们需要 NaN
                    # 强制把 0 或极小值转为 NaN，与 PNG 行为保持一致
                    mask_invalid = (depth <= 1e-6)
                    depth[mask_invalid] = np.nan
                    
                else:
                    # === PNG 模式 ===
                    # 使用 moge.utils.io.read_depth (它会自动处理 Log 解码和 NaN 转换)
                    depth = read_depth(depth_path)

                # --- 🔥 [核心拦截器] 废图过滤 ---
                # 1. 检查是否全空 (这是导致 min=nan 和模型变蓝的元凶)
                if np.isnan(depth).all():
                    raise ValueError(f"Depth map is ALL NaN (Empty): {depth_path}")
                
                # 2. 检查有效像素是否太少 (例如少于 100 个点)
                # 这种图即使能跑，也会导致梯度不稳定，不如扔掉
                valid_count = np.isfinite(depth).sum()
                if valid_count < 100:
                    raise ValueError(f"Too few valid pixels ({valid_count}): {depth_path}")
                
                # --- ✅ 只有“好图”才会运行到这里 ---
                # 打印调试信息 (只打印前 50 个 batch，避免刷屏)
                # 使用 np.nanmin/max 忽略 NaN 进行统计
                if instance.get('batch_id', 0) < 50:
                    d_min = np.nanmin(depth)
                    d_max = np.nanmax(depth)
                    # print(f"✅ PASS: {instance['filename']} | Range: {d_min:.2f} ~ {d_max:.2f}")
                ...
                # ❌ 如果被上面的拦截器抓住，或者文件损坏，会进入这里
                # 打印一条简短的跳过日志 (可选)
                # print(f"⚠️ Skip invalid: {instance.get('filename')} -> {e}")
                
                # 标记为 invalid，DataLoader 后续会自动丢弃这个样本
                instance.update(self.invalid_instance)
    ...
    def _process_instance(self, instance: Dict[str, Union[np.ndarray, str, float, bool]]):
        ...
        # 🔥 改为 linear，大幅降低 CPU 负载
# 🔥 必须用 'bilinear'，它对应 cv2.INTER_LINEAR (双线性插值)
        # tgt_image = warp_perspective(raw_image, transform, tgt_size=(tgt_height, tgt_width), interpolation='bilinear')
        # - Warp depth
        ...
        # 🔥 忽略除以 0 的警告 (因为 0 深度变无穷大是预期的)
        with np.errstate(divide='ignore', invalid='ignore'):
            inv_depth = 1.0 / raw_depth
            warped_inv_depth = warp_perspective(inv_depth, transform, (tgt_height, tgt_width), interpolation='bilinear')
            warped_depth_bilinear = 1.0 / warped_inv_depth
...
# --- 🟢 新代码 (强制截断逻辑) ---
        ...
        # 1. 获取阈值 (从 json 里的 clamp_max_depth 读取，即 500.0)
        hard_limit = self.clamp_max_depth 
        
        # 2. 创建“有效掩码” (Mask)
        # 条件：(是数字) 且 (大于 0.1米) 且 (小于 500米)
        # 注意：这里加 0.1 是为了过滤掉 0 值和极小噪声
        valid_mask = np.isfinite(tgt_depth) & (tgt_depth > 1e-4) & (tgt_depth < hard_limit)
        
        # 3. 强制截断 (Hard Cut-off)
        # 将所有不满足条件的像素直接设为 NaN
        # MoGe 的 Loss 会自动忽略 NaN 区域，不产生梯度
        tgt_depth = np.where(valid_mask, tgt_depth, np.nan)

        # # 4. 更新 Mask (告诉后面的代码哪些地方是有效的)
        tgt_depth_mask_fin = np.isfinite(tgt_depth) 
        tgt_depth_mask_inf = np.isinf(tgt_depth) # 通常全为 False，因为我们把无效的都设为 NaN 了

        # tgt_depth = np.where(valid_mask, tgt_depth, 0.0)
        # tgt_depth_mask_fin = np.isfinite(tgt_depth) & (tgt_depth > 1e-6)
        # tgt_depth_mask_inf = np.zeros_like(tgt_depth, dtype=bool) # 既然截断了，就没有 Inf 了
        ...
        # if instance['batch_id'] < 5:  # 只打印前几个 batch
        #     print(f"DEBUG DEPTH: min={instance['depth'].min()}, max={instance['depth'].max()}")
        return instance
