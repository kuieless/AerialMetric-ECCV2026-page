...
#         pred_depth = pred['depth_metric'] # 获取原始值
#         gt_depth = gt['depth']

#         scale_diff = pred_depth.median() / gt_depth.median()
#         # print(f"\n======== TRUTH REVEALED ========")
#         # print(f"Pred Median: {pred_depth.median().item():.4f}")
#         # print(f"GT   Median: {gt_depth.median().item():.4f}")
#         # print(f"Scale Factor: {scale_diff:.4f} (这就是偏差倍数)")
#         # print(f"Max Value Match?: {pred_depth.max().item()} vs {gt_depth.max().item()}")
#         # print("================================\n")
#         # ==================== 🛠️ 修复 Infinity 开始 ====================
#         # 1. 定义有效范围 (防止 0 或 无穷大 导致的计算溢出)
#         # 室内通常 0.1~10m，室外/航拍通常 0.1~100m (或 80m)
#         min_eval_depth = 1e-3
#         max_eval_depth = 80.0  # 如果你是航拍，这个值可以设大一点，比如 200.0

#         # 2. 生成过滤 Mask
#         # 必须同时满足：GT在范围内、预测值在范围内、且都不是 Inf/NaN
#         valid_mask = (gt_depth > min_eval_depth) & (gt_depth < max_eval_depth) & \
#                      (pred_depth > min_eval_depth) & (pred_depth < max_eval_depth) & \
...
#         #     'rmse': rmse_depth(pred_depth[mask], gt_depth[mask]) # 新增这一行
#         # }
#         metrics['depth_metric'] = {
#             # 注意：这里把 mask 换成了 final_mask
#             'rel': rel_depth(pred_depth[final_mask], gt_depth[final_mask]),
#             'delta1': delta1_depth(pred_depth[final_mask], gt_depth[final_mask]),
#             'rmse': rmse_depth(pred_depth[final_mask], gt_depth[final_mask])
#         }
...
#         mask_valid = mask & (gt_depth > 0)
#         # if mask_valid.sum() > 0:
#             # print(f"\n======== DEBUG INFO ({benchmark_name}) ========")
#             # print(f"\n======== DEBUG INFO ========")
#             # print(f"GT   Depth (Mean): {gt_depth[mask_valid].mean().item():.4f} | Range: {gt_depth[mask_valid].min().item():.4f} - {gt_depth[mask_valid].max().item():.4f}")
#             # print(f"Pred Depth (Mean): {pred_depth[mask_valid].mean().item():.4f} | Range: {pred_depth[mask_valid].min().item():.4f} - {pred_depth[mask_valid].max().item():.4f}")
#             # print(f"Ratio (Pred/GT):   {(pred_depth[mask_valid] / gt_depth[mask_valid]).mean().item():.4f}")
#             # print("==============================================\n")
#         metrics['points_metric'] = {
#             'rel': rel_point(pred_points[mask], gt_points[mask]),
#             'delta1': delta1_point(pred_points[mask], gt_points[mask]),
#             'rmse': rmse_depth(pred_points[mask], gt_points[mask]) # 别忘了加上我们之前定义的rmse
#         }
...
#     return metrics, misc
# 找到 moge/test/metrics.py 文件
# 用这个函数完全替换原有的 compute_metrics 函数
...
def compute_metrics(
    ...
    # ==================== 🛡️ 核心修复：定义全局过滤器 🛡️ ====================
    # 这一步是为了防止 KITTI 天空或无效点导致计算卡死
    # 1. 基础范围过滤
    min_eval = 1e-3
    max_eval = 80.0 # KITTI 标准往往是 80m

    # 2. 深度有效性检查 (必须同时满足 GT 和 Pred 都是有效数值)
    # 注意：我们先检查 depth_metric 是否存在，如果不存在，暂时用 gt_depth 占位来生成 mask
    check_pred = pred['depth_metric'] if 'depth_metric' in pred else gt_depth 
    
    valid_mask = (gt_depth > min_eval) & (gt_depth < max_eval) & \
                 torch.isfinite(gt_depth) & torch.isfinite(check_pred)
    ...
    if pred_depth_scale_invariant is not None:
        ...
        # ⚠️ 增加保护：防止对齐计算卡死
        if gt_depth_lr_masked.numel() > 0 and torch.isfinite(pred_depth_lr_masked).all():
            scale = align_depth_scale(pred_depth_lr_masked, gt_depth_lr_masked, 1 / (gt_depth_lr_masked + 1e-6))
            pred_depth = pred_depth * scale
            ...
            metrics['depth_scale_invariant'] = {'rel': 0.0, 'delta1': 0.0} # 避免报错
    ...
    if pred_depth_affine_invariant is not None:
        ...
        # ⚠️ 增加保护
        if gt_depth_lr_masked.numel() > 0 and torch.isfinite(pred_depth_lr_masked).all():
            scale, shift = align_depth_affine(pred_depth_lr_masked, gt_depth_lr_masked, 1 / (gt_depth_lr_masked + 1e-6))
            pred_depth = pred_depth * scale + shift
            ...
            metrics['depth_affine_invariant'] = {'rel': 0.0, 'delta1': 0.0}
    ...
    if 'disparity_affine_invariant' in pred:
        ...
        pred_disparity_affine_invariant = 1 / (pred['depth_scale_invariant'] + 1e-6)
    elif 'depth_metric' in pred:
        pred_disparity_affine_invariant = 1 / (pred['depth_metric'] + 1e-6)
    else:
        pred_disparity_affine_invariant = None
    ...
    if pred_disparity_affine_invariant is not None:
        ...
        # ⚠️ 增加保护
        mask_align = final_mask # 使用更严格的 Mask
        if mask_align.sum() > 0:
            scale, shift = align_affine_lstsq(pred_disp[mask_align], 1 / (gt_depth[mask_align] + 1e-6))
            pred_disp = pred_disp * scale + shift
            pred_depth = 1 / pred_disp.clamp_min(1 / gt_depth[final_mask].max().item())
            ...
            metrics['disparity_affine_invariant'] = {
             ...
             metrics['disparity_affine_invariant'] = {'rel': 0.0, 'delta1': 0.0}
    ...
    if 'points_metric' in pred and gt['is_metric']:
        ...
        metrics['points_metric'] = {
            'rel': rel_point(pred_points[final_mask], gt_points[final_mask]),
            'delta1': delta1_point(pred_points[final_mask], gt_points[final_mask]),
            'rmse': rmse_depth(pred_points[final_mask], gt_points[final_mask])
        }
    ...
    if pred_points_scale_invariant is not None:
        ...
        # ⚠️ 增加保护
        if gt_points_lr_masked.numel() > 0:
            scale = align_points_scale(pred_points_lr_masked, gt_points_lr_masked, 1 / (gt_points_lr_masked.norm(dim=-1) + 1e-6))
            pred_points = pred_points * scale
    ...
    if pred_points_affine_invariant is not None:
        ...
        if vis and pred_points_aligned is None:
            pred_points_aligned = pred['points_affine_invariant'] 

    # Local points (通常不需要改，如果卡住再改)
    if 'segmentation_mask' in gt and 'points' in gt and any('points' in k for k in pred.keys()):
        pred_points = next(pred[k] for k in pred.keys() if 'points' in k)
        gt_points = gt['points']
        segmentation_mask = gt['segmentation_mask']
        ...
        local_points_metrics = []
        for _, seg_id in segmentation_labels.items():
            valid_mask = (segmentation_mask == seg_id) & final_mask # Use Final Mask
            ...
            # Skip if NaN
            if not torch.isfinite(pred_points_masked).all(): continue

            pred_points_masked_lr = pred_points[lr_index][(segmentation_mask_lr == seg_id) & lr_mask]
            gt_points_masked_lr = gt_points[lr_index][(segmentation_mask_lr == seg_id) & lr_mask]
            
            if pred_points_masked_lr.numel() == 0: continue

            diameter = (gt_points_masked.max(dim=0).values - gt_points_masked.min(dim=0).values).max()
            scale, shift = align_points_scale_xyz_shift(pred_points_masked_lr, gt_points_masked_lr, 1 / (diameter.expand(gt_points_masked_lr.shape[0]) + 1e-6))
            pred_points_masked = pred_points_masked * scale + shift
