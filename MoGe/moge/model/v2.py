...
class MoGeModel(nn.Module):
    ...
    def __init__(self, 
        ...
        **deprecated_kwargs
        ...
        # # ❄️ [新增] 真正的冻结骨干网络代码
        # # =======================================================
        # print("❄️  Freezing Backbone (Encoder) parameters...")
 ...
 # 在 def forward(...) 的末尾
        return_dict = {
            ...
            'normal': normal,
            'mask': mask,
            'metric_scale': metric_scale  # 只需要这四个
        }
...
###### 通过三角函数计算fovx fovy
        ...
        return_dict = {
        'points': points,
        'intrinsics': intrinsics,     # <--- ADD THIS BACK
        'depth': depth,               # <--- ADD THIS BACK
        'mask': mask_binary,          # <--- USE mask_binary HERE (it is the processed mask)
        'normal': normal,
        'metric_scale': metric_scale  # <--- YOUR NEW ADDITION
        }
        return_dict = {k: v for k, v in return_dict.items() if v is not None}
