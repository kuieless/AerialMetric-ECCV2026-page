...
PATHS_MOGE_LARGE = [
        ...
        'pred_dir': '/home/data1/szq/Megadepth/benchmarkmodel/Moge2/4scene-infer-out/Arc de Triomphe',
        'gt_dir': '/home/data1/szq/Megadepth/benchmarkmodel/Moge2/4scene-infer-4k5-out/Arc de Triomphe'
    },
            {
        'name': 'Brandenburg Gate',
        'pred_dir': '/home/data1/szq/Megadepth/benchmarkmodel/Moge2/4scene-infer-out/Brandenburg Gate',
        'gt_dir': '/home/data1/szq/Megadepth/benchmarkmodel/Moge2/4scene-infer-4k5-out/Brandenburg Gate'
    },
            {
        'name': 'tennis',
        'pred_dir': '/home/data1/szq/Megadepth/benchmarkmodel/Moge2/4scene-infer-out/tennis',
        'gt_dir': '/home/data1/szq/Megadepth/benchmarkmodel/Moge2/4scene-infer-4k5-out/tennis'
    },
            {
        'name': 'tennis2',
        'pred_dir': '/home/data1/szq/Megadepth/benchmarkmodel/Moge2/4scene-infer-out/tennis2',
        'gt_dir': '/home/data1/szq/Megadepth/benchmarkmodel/Moge2/4scene-infer-4k5-out/tennis2'
    },
...
class PairedNpyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ==========================================================
        # 修复核心：检查维度，如果是 (H, W, 3) 则只取 Z 通道
        # ==========================================================
        # 处理预测值 (Pred)
        if pred_np.ndim == 3:
            if pred_np.shape[-1] == 3: 
                # Case: (H, W, 3) -> 取最后一个通道 Z
                pred_np = pred_np[:, :, 2]
            elif pred_np.shape[0] == 1:
                # Case: (1, H, W) -> 降维
                pred_np = pred_np.squeeze(0)
            elif pred_np.shape[0] == 3:
                # Case: (3, H, W) -> 比较少见，假设第3个是Z
                pred_np = pred_np[2, :, :]

        # 处理真值 (GT) - 同样的逻辑，以防 GT 也是点云格式
        if gt_np.ndim == 3:
            if gt_np.shape[-1] == 3:
                gt_np = gt_np[:, :, 2]
            elif gt_np.shape[0] == 1:
                gt_np = gt_np.squeeze(0)
            elif gt_np.shape[0] == 3:
                gt_np = gt_np[2, :, :]
        ...
        # 确保现在的形状严格是 (H, W)
        # 如果还有多余维度 (比如 (H, W, 1))，再 squeeze 一次
        if pred.ndim == 3: pred = pred.squeeze()
        if gt.ndim == 3: gt = gt.squeeze()
